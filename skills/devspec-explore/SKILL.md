...
3. **Scale guidance to complexity**:
   ...
   - **3+ options**: You MUST use `devspec_ask_questions` for the selection - no exceptions. Write a brief analysis paragraph inline, then call the tool. This applies even when you have a strong recommendation, even when options can be combined, and even when framing them as "analysis" rather than "questions." The tool is part of the analytical output, not an alternative to it. Include pre-computed `research`, `reflection`, and `recommendation` fields in the payload. Tell the user to run `/answers` when done.
